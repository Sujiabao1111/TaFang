"use strict";
cc._RF.push(module, 'dcc33TmOpxNP6hecSN5szAc', 'Ajax');
// Script/server/ServerMgr/Ajax.ts

"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var util_1 = require("../../util/util");
var UrlConst_1 = require("../UrlConst");
var AppInfo_1 = require("../xmsdk_cocos/Config/AppInfo");
var md5_1 = require("../xmsdk_cocos/Utils/md5");
var Ajax = /** @class */ (function () {
    function Ajax() {
    }
    Ajax.send = function (config) {
        var tenurl = config.url;
        if (!/^http/.test(config.url)) {
            config.url = AppInfo_1.AppInfo.appHost + config.url;
        }
        ...
        if (!data.phead) {
            var now = Date.now();
            data.phead = AppInfo_1.getPhead();
            data.phead.wversion = AppInfo_1.WebVersionCode;
            data.sign = md5_1.md5.md5("prdId=" + data.phead.prdid + "&phoneId=" + data.phead.phoneid + "&timestamp=" + now + "&key=xkX2Ab1P3KuI214V");
            data.timestamp = now;
        }
        /*let listener = {
            onSuccess: config.onSuccess || function (res) { },
    ...
    Ajax.getdata = function (config) {
        // console.log("\n----getdata-------" + config.url)
        var data = [];
        if (config.url == UrlConst_1.UrlConst.onPrizeGetRewardMain) {
        ...
        else if (config.url == UrlConst_1.UrlConst.btnRandomRedCount) {
            data = { "code": 0, "message": "success", "data": { "remainingTimes": 97 } };
        }
        else if (config.url == UrlConst_1.UrlConst.earnProgressIndex) {
            data = { "code": 0, "message": "success", "data": { "point": 171428, "nextGear": 175000, "reward": 500, "canReceiveTimes": 7 } };
        }
        else if (config.url == UrlConst_1.UrlConst.goldWheel_index) {
        ...
        else if (config.url == UrlConst_1.UrlConst.sign_main) {
            //签到
            var state = util_1.default.canSinge();
            var lengsig = util_1.default.singlen();
        ...
        else if (config.url == UrlConst_1.UrlConst.newPlayerTaskData) {
        ...
        else if (config.url == UrlConst_1.UrlConst.wallet_main2) {
        ...
        else if (config.url == UrlConst_1.UrlConst.heavenCoin_main) {
            data = { "code": 0, "message": "success", "data": { "list": [], "remainingTimes": 85 } };
        }
        else if (config.url == UrlConst_1.UrlConst.treasureBox_residual) {
            data = { "code": 0, "message": "success", "data": { "times": 17, "coin": 841 } };
        }
        else if (config.url == UrlConst_1.UrlConst.treasureBox_get2) {
            data = { "code": 0, "message": "success", "data": { "point": 253 } };
        }
        else if (config.url == UrlConst_1.UrlConst.earnProgressReceive) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.gameLevelReport) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.task_day_main) {
            //每日任务
            data = {
            ...
            var sts = [2, 2, 2, 2, 2, 2, 4, 4];
            for (var i = 0; i < data.data.list.length; i++) {
                var isd = data.data.list[i].id;
                // console.log('###########' + JSON.stringify(data.data.list[i].id))
                var state = util_1.default.getInt(isd, sts[i]);
                data.data.list[i].buttonType = state;
            }
        }
        else if (config.url == UrlConst_1.UrlConst.achievement_main) {
            ...
            var sts = [2, 2, 2, 2];
            for (var i = 0; i < data.data.list.length; i++) {
                var isd = data.data.list[i].id + "cj";
                //console.log('###########'+JSON.stringify(data.data.list[i].id))
                var state = util_1.default.getInt(isd, sts[i]);
                data.data.list[i].buttonType = state;
            }
        }
        else if (config.url == UrlConst_1.UrlConst.newBigWheel_index) {
        ...
        else if (config.url == UrlConst_1.UrlConst.heavenCoin_get) {
            data = { "code": 0, "message": "success", "data": { "id": "1414875119070871553", "point": 23, "distanceTime": "0" } };
        }
        else if (config.url == UrlConst_1.UrlConst.heavenCoin_receive) {
            data = { "code": 0, "message": "已领取该空降金币", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.watchVideoAddBattery) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.getOnLinePrize) {
            data = { "code": 0, "message": "success", "data": { "point": 90, "leftTime": "0" } };
        }
        else if (config.url == UrlConst_1.UrlConst.btnRandomRedGet) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.goldWheel_action) {
            //转盘结果
            data = { "code": 0, "message": "success", "data": { "id": "1414895542793797633", "reward": { "id": "105", "value": 5, "type": 1 } } };
        }
        else if (config.url == UrlConst_1.UrlConst.goldWheel_checkIn) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.task_day_commonGet) {
            data = { "code": 0, "message": "success", "data": null };
            //xiu
            //
            util_1.default.setInt(config.data.id, 4);
            //console.log("--00000000000--------############# : " + JSON.stringify(config) )
        }
        else if (config.url == UrlConst_1.UrlConst.newPlayerTaskGet) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.kingPaoTaskData) {
        ...
        else if (config.url == UrlConst_1.UrlConst.kingPaoProgress) {
            data = { "code": 0, "message": "success", "data": { "percent": 10, "process": 0, "processTarget": 3, "status": 1, "sign": false } };
        }
        else if (config.url == UrlConst_1.UrlConst.videoCardOk) {
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.wallet_record) {
            data = { "code": 0, "message": "success", "data": { "list": [] } };
        }
        else if (config.url == UrlConst_1.UrlConst.onPrizeGetRewardGet) {
            //在线奖励领取
            data = { "code": 0, "message": "success", "data": null };
        }
        else if (config.url == UrlConst_1.UrlConst.newBigWheel_checkIn) {
            data = { "code": 0, "message": "success", "data": { "rewardPhoneFragments": 1.0 } };
        }
        else if (config.url == UrlConst_1.UrlConst.newBigWheel_action) {
            data = { "code": 0, "message": "success", "data": { "id": "107", "rewardValue": 3.0, "doubleId": "1414901818462437378", "doubleValue": 0 } };
        }
        else if (config.url == UrlConst_1.UrlConst.gameLevelIndex) {
            data = { "code": 0, "message": "success", "data": { "rewardList": [] } };
        }
        else if (config.url == UrlConst_1.UrlConst.sign_videoGet) {
            //签到回调
            data = { "code": 0, "message": "已签到", "data": null };
            util_1.default.singtoday();
        }
        else if (config.url == UrlConst_1.UrlConst.gameLevelCompleted) {
            data = { "code": 0, "message": "success", "data": { "rewardList": [] } };
        }
        else if (config.url == UrlConst_1.UrlConst.achievement_commonGet) {
            data = { "code": 0, "message": "领取成功", "data": null };
            var isd = config.data.id + "cj";
            //console.log('###########'+JSON.stringify(data.data.list[i].id))
            util_1.default.setInt(isd, sts[i], 4);
        }
        var listener = {
            onSuccess: config.onSuccess || function (res) { },
            onFail: config.onFail || function (res) { },
