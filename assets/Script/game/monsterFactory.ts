...
export default class monsterFactory extends cc.Component {
    ...
    monsterBruise(atk: number, crit: number = 1) {
        ...
        atk *= crit;
        //增能
        let energizedNum: number = this._userData.prop[propType.energized - 1].use ? 1 : 0;
        //电击
        let shockNum: number = this._userData.prop[propType.shock - 1].use ? .2 : 0;
        //伤害
        ...
        if (this.monsterHp <= 0) {
            this.clearMonster();
            // console.log("怪兽死亡");
            cc.game.emit(NameTs.Game_Kills_Updata, true);
    ...
    update(dt) {
        ...
        if (this._userData.prop[propType.frozen - 1].use == propState.start && this.walkNo > 0 && !this.isFrozen) {
            this.isFrozen = true;
            this.openFrozen();
            this.stopWalk();
        } else if (this._userData.prop[propType.frozen - 1].use == propState.end && this.isFrozen) {
            this.closeFrozen();
            this.resumeWalk();
        }
        if (this._userData.prop[propType.shield - 1].use == propState.start && !this.isShield) {
            this.isShield = true;
