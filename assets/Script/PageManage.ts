...
export default class PageManage extends cc.Component {
   ...
   * @param 加载中......
   */
    public Loading(timeOut: number = 9999) {
        let loadingParent = cc.find("Canvas/Loading");
        if (loadingParent.childrenCount <= 0) {
            let item = null;
            if (this.pageOpen.has("Loading")) {
                let item: cc.Node = cc.instantiate(this.pageOpen.get("Loading"));
                item.setParent(loadingParent);
                item.getComponent(Loading).show(timeOut);
            } else {
                cc.resources.load("prefab/effect/Loading", cc.Prefab, (err, res) => {
                    item = cc.instantiate(res);
                    this.pageOpen.set(item.name, item);
                    item.setParent(loadingParent);
                    item.getComponent(Loading).show(timeOut);
                })
            }
